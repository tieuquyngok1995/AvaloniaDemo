...
namespace ConfigGenerator.Converters;

/// <summary>
/// 各種型変換用のコンバーターをまとめたクラスです。
/// アイコンのキーからStreamGeometryへの変換や、
/// メニューの幅を切り替えるためのコンバーターなどを提供します。
/// </summary>
public class TypeConverters
{
    // アイコンが見つからない場合のデフォルトのStreamGeometryデータ
    ...
    /// <summary>
    /// アイコンキーからStreamGeometryへの変換を行うコンバーターです。
    /// </summary>
    public static FuncValueConverter<string, StreamGeometry> IconConverter { get; } =
        new(iconKey =>
        {
            if (iconKey is null)
            {
                return StreamGeometry.Parse(StreamGeometryNotFound);
            }
    ...
    /// <summary>
    /// メニューの開閉状態に応じて幅を切り替えるコンバーターです。
    /// </summary>
    public static readonly MenuPaneWidthConverter MenuPaneWidthConverter = new();
}

/// <summary>
/// メニューの開閉状態（IsPaneOpen）に応じて幅を返すコンバーターです。
/// 開いている場合は300.0、閉じている場合は42.0を返します。
/// </summary>
